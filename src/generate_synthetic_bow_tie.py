...
def build_network_structure(
    ...
    ## out nodes
    if num_out > num_output_nodes: # tree-like out component
        ...
        while not nx.is_weakly_connected(graph.subgraph(core_nodes + out_nodes)):

            n1 = np.random.choice(core_nodes)

            n2 = np.random.choice(out_nodes)
...
def parse_args():
    ...
    parser.add_argument("--num_output_nodes", 
        dest="num_output_nodes", type=int, default=3,
